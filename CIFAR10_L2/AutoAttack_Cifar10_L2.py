...
def main():
    ...
    parser.add_argument("--exp-name", type=str, default="AA__CIFAR10_L2")
    ...
    ########################### Creating model ############################
    name_model, threshold ='Wang2023Better_WRN-70-16', 0.25 #Rank 1
    directory_WS = "models_WS"

    if not os.path.exists(directory_WS):
        os.makedirs(directory_WS, exist_ok=True)

    model_original = load_model(model_name=name_model, dataset='cifar10', threat_model='L2')
    ...
    results = AutoAttack_Wrapper(model, device, x_test, y_test, args, logger, sub_dir_path=result_sub_dir, start_batch=args.start_batch,
                                end_batch=args.end_batch, checkpoint_path=ckpt_path, distance="L2", epsilon=0.5, attack_type=None, batch_size=args.batch_size)
